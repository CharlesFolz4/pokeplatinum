...
typedef struct VillaFurnitureCollision {
    u16 furniture;
    u16 isCollisionEnabled;
    VecFx32 position; // specifies the bottom-right point of the prop model
    FurnitureCollisionBoundingBox collisionBounds;
} VillaFurnitureCollision;
...
typedef struct VillaDynamicMapData {
    ...
    VillaPersistedFeature *persistedFeature;
    VillaFurnitureProp furnitureProps[FURNITURE_SLOTS];
} VillaDynamicMapData;

static void LoadFurniturePropModel(VillaDynamicMapData *villaDynamicData, const enum VillaFurniture furniture);
static BOOL FieldSystem_OwnsVillaFurniture(FieldSystem *fieldSystem, enum VillaFurniture furniture);
static BOOL CheckFurnitureCollision(int tileX, int tileZ, const VillaFurnitureCollision *furnitureCollision, FieldSystem *fieldSystem);
...
void Villa_DynamicMapFeaturesInit(FieldSystem *fieldSystem)
{
    ...
    VillaPersistedFeature *villaPersistedFeatures = PersistedMapFeatures_GetBuffer(persistedFeatures, DYNAMIC_MAP_FEATURES_VILLA);
    ...
    villaDynamicData->persistedFeature = villaPersistedFeatures;
...
{
    int i;
    const VillaFurnitureCollision *furnitureCollision = sVillaFurnitureCollision;

    for (i = 0; i < FURNITURE_SLOTS; i++, furnitureCollision++) {
        if (CheckFurnitureCollision(tileX, tileZ, furnitureCollision, fieldSystem) == TRUE) {
            *isColliding = TRUE;
            return TRUE;
        }
    }
...
BOOL FieldSystem_TrySetVillaFurnitureScript(FieldSystem *fieldSystem, const int tileX, const int tileZ, const int direction)
{
    ...
    if (PersistedMapFeatures_GetID(persistedFeatures) == DYNAMIC_MAP_FEATURES_VILLA) {
        int i;
        const VillaFurnitureCollision *furnitureCollision = sVillaFurnitureCollision;

        for (i = 0; i < FURNITURE_SLOTS; i++, furnitureCollision++) {
            if (CheckFurnitureCollision(tileX, tileZ, furnitureCollision, fieldSystem) == TRUE) {
                u32 furnitureScript = sVillaFurnitureScriptIDs[furnitureCollision->furniture];
...
static void LoadFurniturePropModel(VillaDynamicMapData *villaDynamicData, const enum VillaFurniture furniture)
{
    ...
    int modelID = sVillaFurnitureModelIDs[furniture];
    const VillaFurnitureCollision *furnitureCollision = sVillaFurnitureCollision;
    FieldSystem *fieldSystem = villaDynamicData->fieldSystem;

    for (int i = 0; i < FURNITURE_SLOTS; i++, furnitureCollision++) {
        if (furnitureCollision->furniture == furniture) {
            ...
            for (int j = 0; j < FURNITURE_SLOTS; j++) {
                if (!furnitureProp->isLoaded) {
                    ...
                    furnitureProp->propIdx = MapPropManager_LoadOne(fieldSystem->mapPropManager, fieldSystem->areaDataManager, modelID, &furnitureCollision->position, &rotation, fieldSystem->mapPropAnimMan);
...
static BOOL CheckFurnitureCollision(int tileX, int tileZ, const VillaFurnitureCollision *furnitureCollision, FieldSystem *fieldSystem)
{
    const FurnitureCollisionBoundingBox *collisionBounds = &furnitureCollision->collisionBounds;

    if (furnitureCollision->isCollisionEnabled == TRUE && FieldSystem_OwnsVillaFurniture(fieldSystem, furnitureCollision->furniture) == TRUE) {
...
static const VillaFurnitureCollision sVillaFurnitureCollision[FURNITURE_SLOTS] = {
    {
        .furniture = VILLA_FURNITURE_TABLE,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(12),
            .y = 0,
            .z = ALSO_MAP_OBJECT_COORD_TO_FX32(7),
        },
        .collisionBounds = {
            .left = 11,
            .top = 5,
    ...
    {
        .furniture = VILLA_FURNITURE_BIG_SOFA,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(15),
            .y = 0,
            .z = ALSO_MAP_OBJECT_COORD_TO_FX32(7),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_SMALL_SOFA,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(13),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(10),
        },
        .collisionBounds = {
            .left = 11,
            .top = 9,
    ...
    {
        .furniture = VILLA_FURNITURE_BED,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(18),
            .y = 0,
            .z = ALSO_MAP_OBJECT_COORD_TO_FX32(8),
        },
        .collisionBounds = {
            .left = 18,
            .top = 6,
    ...
    {
        .furniture = VILLA_FURNITURE_NIGHT_TABLE,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(20),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(7),
        },
        .collisionBounds = {
            .left = 17,
            .top = 6,
    ...
    {
        .furniture = VILLA_FURNITURE_TV,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(12),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
            .left = 11,
            .top = 3,
    ...
    {
        .furniture = VILLA_FURNITURE_AUDIO_SYSTEM,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(14),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_BOOKSHELF,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(17),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_RACK,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(19),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_HOUSEPLANT,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(1),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_PC_DESK,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(2),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(8),
        },
        .collisionBounds = {
            .left = 1,
            .top = 7,
    ...
    {
        .furniture = VILLA_FURNITURE_MUSIC_BOX,
    ...
    {
        .furniture = VILLA_FURNITURE_POKEMON_BUST,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(2),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_POKEMON_BUST_SILVER,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(6),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(3),
        },
        .collisionBounds = {
    ...
    {
        .furniture = VILLA_FURNITURE_PIANO,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(2),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(5),
        },
        .collisionBounds = {
            .left = 1,
            .top = 4,
    ...
    {
        .furniture = VILLA_FURNITURE_GUEST_SET,
        .isCollisionEnabled = TRUE,
        .position = {
            .x = ALSO_MAP_OBJECT_COORD_TO_FX32(6),
            .y = 0,
            .z = ALSO_MAP_OBJECT_COORD_TO_FX32(10),
        },
        .collisionBounds = {
            .left = 4,
            .top = 9,
    ...
    {
        .furniture = VILLA_FURNITURE_WALL_CLOCK,
        .isCollisionEnabled = FALSE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(11),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(1),
        },
        .collisionBounds = COLLISION_BOUNDS_NONE,
    },
    {
        .furniture = VILLA_FURNITURE_MASTERPIECE,
        .isCollisionEnabled = FALSE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(8),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(1),
        },
        .collisionBounds = COLLISION_BOUNDS_NONE,
    ...
    {
        .furniture = VILLA_FURNITURE_TEA_SET,
        .isCollisionEnabled = FALSE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(6),
            .y = 0,
    ...
    {
        .furniture = VILLA_FURNITURE_CHANDELIER,
        .isCollisionEnabled = FALSE,
        .position = {
            .x = MAP_OBJECT_COORD_TO_FX32(7),
            .y = 0,
            .z = MAP_OBJECT_COORD_TO_FX32(6),
        },
        .collisionBounds = COLLISION_BOUNDS_NONE,
    },
};
